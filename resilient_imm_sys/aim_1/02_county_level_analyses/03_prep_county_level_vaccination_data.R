...
# load data
full_data <- readRDS(file = paste0(prepped_data_dir, "04_extracted_county_level_data.RDS"))
...
for (s in states) {
  ...
  if (s=="Washington"){
    data <- full_data %>% filter(state==s) %>% 
     filter(year %in% c(2015, 2017))}  else {
    data <- full_data %>% filter(state==s) %>%
  ...
  if (s=="Washington"){
    tmpData$change <- (tmpData$`2017` - tmpData$`2015`)
  } else {
  ...
  # for Virginia, small counties do not have data reported for some years, so will drop rows where change cannot be calculate
  if (s=="Virginia"){
  ...
  saveRDS(tmpData, file=paste0(prepped_data_dir, "prepped_state_level_data/01_vaccination_data_individual_states/", file_name_state, ".RDS"))
  write.csv(tmpData, file=paste0(prepped_data_dir, "prepped_state_level_data/01_vaccination_data_individual_states/", file_name_state, ".csv"))
  ...
  # save bound data
  saveRDS(prepped_county_data, file=paste0(prepped_data_dir, "05_prepped_county_level_vaccination_data.RDS"))
